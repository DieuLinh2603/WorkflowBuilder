...
public class DataPipelineService {
    ...
    @Transactional public Map<String,Object> upload(UUID id,String alias,MultipartFile upload){DataPipeline p=owned(id);if(upload.isEmpty())throw new IllegalArgumentException("CSV file is empty");try{byte[] bytes=upload.getBytes();String hash=HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes));int number=files.findByPipelineIdAndSourceAliasOrderByVersionNumberDesc(id,alias).stream().findFirst().map(x->x.getVersionNumber()+1).orElse(1);PipelineFileVersion f=files.save(PipelineFileVersion.builder().pipeline(p).sourceAlias(alias).versionNumber(number).originalName(Optional.ofNullable(upload.getOriginalFilename()).orElse("data.csv")).contentType(upload.getContentType()).contentBytes(bytes).checksum(hash).uploadedBy(currentUser.user()).build());return Map.of("id",f.getId(),"sourceAlias",alias,"versionNumber",number,"fileName",f.getOriginalName(),"checksum",hash);}catch(Exception ex){throw new IllegalArgumentException("Cannot store CSV: "+ex.getMessage(),ex);}}
