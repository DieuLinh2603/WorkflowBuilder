...
public class PipelineDefinitionEngine {
    ...
    public ExecutionResult execute(DataPipeline pipeline) {
        try {
            ...
            PreparedData prepared=prepare(pipeline,definition);
            ...
            rows=projectAndValidate(rows,schema,pipeline.getBusinessKey()); guard(rows,"VALIDATION");
    ...
    private PreparedData prepare(DataPipeline pipeline,Map<String,Object> definition) {
        ...
        for(Map<String,Object> source:sourceDefs){String alias=req(source,"alias");if(loaded.containsKey(alias))throw new StageException("SOURCE","Duplicate alias: "+alias);List<Map<String,Object>> rows=loadSource(pipeline,source);loaded.put(alias,rows);input+=rows.size();}
        List<Map<String,Object>> rows=new ArrayList<>(loaded.get(sourceDefs.get(0).get("alias").toString()));
        for(Map<String,Object> join:listOfMaps(definition.get("joins"))){String right=req(join,"rightAlias");if(!loaded.containsKey(right))throw new StageException("JOIN","Unknown rightAlias: "+right);rows=join(rows,loaded.get(right),join);guard(rows,"JOIN");}
        rows=transform(rows,listOfMaps(definition.get("transforms")));
        return new PreparedData(rows,input,sourceDefs.size());
    }

    private List<Map<String,Object>> loadSource(DataPipeline pipeline,Map<String,Object> source) {
        String type=String.valueOf(source.getOrDefault("type","")).toUpperCase(Locale.ROOT);
        try { return switch(type){case "CSV"->loadCsv(pipeline,source);case "REST"->loadRest(source);case "POSTGRESQL"->loadPostgres(source);default->throw new StageException("SOURCE","Unsupported source type: "+type);}; }
    ...
    private List<Map<String,Object>> loadRest(Map<String,Object> source)throws Exception{
        ...
        for(int page=0;page<1000;page++){
            ...
            JsonNode root=mapper.readTree(response.body()),records=path(root,String.valueOf(source.getOrDefault("recordPath","")));
            if(!records.isArray())throw new StageException("SOURCE","recordPath must point to an array");
            int before=out.size();
    ...
    private List<Map<String,Object>> transform(List<Map<String,Object>> input,List<Map<String,Object>> defs){
