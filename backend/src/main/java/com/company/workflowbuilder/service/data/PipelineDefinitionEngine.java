...
public class PipelineDefinitionEngine {
    ...
    private List<Map<String,Object>> loadCsv(DataPipeline pipeline,Map<String,Object> source){UUID id=UUID.fromString(req(source,"fileVersionId"));PipelineFileVersion file=files.findById(id).orElseThrow(()->new StageException("SOURCE","Không tìm thấy phiên bản file CSV"));if(file.getPipeline()!=null&&pipeline.getId()!=null&&!pipeline.getId().equals(file.getPipeline().getId()))throw new StageException("SOURCE","File CSV thuộc pipeline khác");return parseCsv(decodeCsv(file.getContentBytes()),String.valueOf(source.getOrDefault("delimiter","AUTO")));}
    private String decodeCsv(byte[] bytes){
        if(bytes==null||bytes.length==0)throw new StageException("SOURCE","File CSV trống");
    ...
    private List<Map<String,Object>> parseCsv(String content,String delimiter){
        char sep=csvDelimiter(content,delimiter);List<List<String>> lines=new ArrayList<>();List<String> line=new ArrayList<>();StringBuilder cell=new StringBuilder();boolean quoted=false;
        for(int i=0;i<content.length();i++){char c=content.charAt(i);if(c=='"'){if(quoted&&i+1<content.length()&&content.charAt(i+1)=='"'){cell.append('"');i++;}else quoted=!quoted;}else if(c==sep&&!quoted){line.add(cell.toString());cell.setLength(0);}else if((c=='\n'||c=='\r')&&!quoted){if(c=='\r'&&i+1<content.length()&&content.charAt(i+1)=='\n')i++;line.add(cell.toString());cell.setLength(0);if(line.stream().anyMatch(v->!v.isBlank()))lines.add(line);line=new ArrayList<>();}else cell.append(c);}
        if(quoted)throw new StageException("SOURCE","File CSV có dấu ngoặc kép chưa đóng");
        line.add(cell.toString());if(line.stream().anyMatch(v->!v.isBlank()))lines.add(line);if(lines.isEmpty())throw new StageException("SOURCE","File CSV không có dữ liệu");
        List<String> headers=lines.get(0).stream().map(String::trim).toList();if(headers.stream().anyMatch(String::isBlank))throw new StageException("SOURCE","Tên cột CSV không được để trống");if(new HashSet<>(headers).size()!=headers.size())throw new StageException("SOURCE","Tên cột CSV bị trùng");
        List<Map<String,Object>> out=new ArrayList<>();for(int i=1;i<lines.size();i++){Map<String,Object> row=new LinkedHashMap<>();for(int j=0;j<headers.size();j++)row.put(headers.get(j),j<lines.get(i).size()?lines.get(i).get(j):"");out.add(row);guard(out,"SOURCE");}return out;
    }
    private char csvDelimiter(String content,String configured){
        if(configured!=null&&!configured.isBlank()&&!"AUTO".equalsIgnoreCase(configured)){if("\\t".equals(configured))return '\t';return configured.charAt(0);}
        String first=content.lines().findFirst().orElse("");char best=',';int count=-1;for(char candidate:new char[]{',',';','\t'}){int current=0;boolean quoted=false;for(int i=0;i<first.length();i++){char c=first.charAt(i);if(c=='"')quoted=!quoted;else if(c==candidate&&!quoted)current++;}if(current>count){best=candidate;count=current;}}return best;
    }
