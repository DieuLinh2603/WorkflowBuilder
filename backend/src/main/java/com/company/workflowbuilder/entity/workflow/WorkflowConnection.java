...
public class WorkflowConnection {
    ...
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "from_step_id", nullable = false)
    private WorkflowStep fromStep;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "to_step_id", nullable = false)
    private WorkflowStep toStep;
    @Enumerated(EnumType.STRING) @Column(nullable = false)
    private ConnectionType type;
    @Enumerated(EnumType.STRING) @Column(name = "logical_operator", nullable = false)
    @Builder.Default private LogicalOperator logicalOperator = LogicalOperator.AND;
    @Column(nullable = false) @Builder.Default private int priority = 100;
