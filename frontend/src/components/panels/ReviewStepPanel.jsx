...
const DEFAULTS={mode:'MANUAL',approverMode:'FIXED_USER',actorUserIds:[],fixedUserEmail:'',actorRole:'',dynamicActorSource:'REQUEST_CREATOR_MANAGER',reviewContent:'REQUEST_CONTENT',commentRequired:true,forwardReviewHandoff:false,resultMode:'REQUIRE_APPROVAL',completionMode:'ANY',completionPercentage:50,deadlineHours:48,deadlineDate:null};
...
export default function ReviewStepPanel({workflowId,step,onClose,onDelete}){
 ...
 return <PanelFrame icon={Search} title="Cấu hình: Review Step" onClose={onClose} onDelete={onDelete} footer={<button onClick={save} disabled={saving} className="btn-primary w-full py-2.5 text-sm disabled:opacity-60">{saving?'Đang lưu...':'Lưu cấu hình'}</button>}>
  <div className="space-y-6">
   ...
   <section className="rounded-xl border border-violet-200 bg-violet-50 p-4"><SectionLabel>Bàn giao kết quả Review</SectionLabel><Toggle checked={!!config.forwardReviewHandoff} onChange={v=>setConfig(c=>({...c,forwardReviewHandoff:v}))} label="Gửi dữ liệu đã review cho người xử lý bước sau"/><p className="mt-2 text-[11px] leading-5 text-violet-700">Khi bật, task phía sau sẽ thấy dữ liệu được phân loại PASS/FAIL, nhận xét và các mục kết quả Reviewer tự tạo. Gói bàn giao chỉ dùng để đọc và không tự động trở thành điều kiện rẽ nhánh.</p></section>
