...
export default function EndStepPanel({workflowId,step,onClose,onDelete}){
 ...
 return <PanelFrame icon={CircleStop} title="Cấu hình: End Step" onClose={onClose} onDelete={onDelete} footer={<div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg border border-grayBorder px-4 py-2.5 text-sm text-slate-600">Hủy</button><button type="button" onClick={save} disabled={saving} className="btn-primary min-w-[150px] py-2.5 text-sm disabled:opacity-60">{saving?'Đang lưu...':'Lưu cấu hình'}</button></div>}>
