...
export default function CatalogPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return workflows.filter(workflow => `${workflow.name} ${workflow.description || ''} ${workflow.module || ''}`.toLocaleLowerCase('vi').includes(keyword));
  }, [query, workflows]);
  ...
  return <div className="mx-auto w-full max-w-[1240px] space-y-7">
    <header className="flex flex-wrap items-end justify-between gap-5">
      ...
      <div className="relative w-full sm:w-80"><Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={event => setQuery(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100" placeholder="Tìm loại yêu cầu..."/></div>
    </header>
